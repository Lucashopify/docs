...
description: How to turn on the Split Tester script in your Shopify theme — and how to verify it is working.
---

## What the app embed does

The app embed is a JavaScript file that Split Tester adds to every page of your storefront through Shopify's Theme App Extension system. It is responsible for:
...
4. Find **Split Tester** in the list
5. Toggle it **on**
...
After enabling the embed, visit any page on your storefront. Then return to the Split Tester dashboard and open **Get Started**. Step 1 shows a green checkmark once the dashboard receives its first event from your store — this usually takes less than 30 seconds after your first page visit.
...
- **Served from Shopify's CDN** — the script is part of a Shopify Theme App Extension, so it is hosted and delivered by Shopify's own infrastructure, not Split Tester's servers
...
For Split Tester's own theme tests, the embed handles this internally by running the variant assignment before the theme preview redirect fires, then preserving the assignment in the `spt_asgn` cookie.
...
- Check the browser console for JavaScript errors from Split Tester scripts
