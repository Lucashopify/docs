...
description: How Split Tester determines whether results are real, including p-values, confidence, SRM, and peeking bias.
---
...
Statistical significance is a measure of how likely the observed difference is due to chance. Split Tester uses a **two-proportion z-test** to compute a p-value for each treatment variant compared to control.
...
Split Tester uses a two-proportion z-test:
...
Split Tester uses **95% confidence** (α = 0.05) as the default threshold. This means:
...
Power is the probability of detecting a real effect when one exists. Split Tester targets **80% power** in its sample size guidance, which is the industry standard.
...
### How Split Tester detects SRM

Split Tester runs a chi-squared test on the visitor counts after each variant reaches 100 visitors:
...
### Split Tester's minimum runtime

Split Tester shows a minimum runtime warning for experiments shorter than 7 days. The conclusion banner only becomes active after:
